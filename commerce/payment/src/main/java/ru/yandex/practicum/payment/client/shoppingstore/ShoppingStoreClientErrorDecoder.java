...
@Slf4j
@Component
public class ShoppingStoreClientErrorDecoder implements ErrorDecoder {

    private final ErrorDecoder defaultDecoder = new Default();

    private final ObjectMapper objectMapper;

    public ShoppingStoreClientErrorDecoder(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    @Override
    public Exception decode(String methodKey, Response response) {
        log.debug("ShoppingStore methodKey: {}, response: {}", methodKey, response);

        try {
            if (methodKey.contains("getProductById") && response.status() == 404) {
                ApiError error = parseErrorBody(response);
                return new ProductNotFoundException(error.getMessage());
            }
            ...
            log.warn("Сломался в ShoppingStoreClientErrorDecoder, methodKey {}", methodKey, e);
        }
    ...
    private ApiError parseErrorBody(Response response) {
        try {
            ...
            log.warn("Сломался в ShoppingStoreClientErrorDecoder на парсинге объекта response {}", response, e);
        }
        return null;
    }
}
